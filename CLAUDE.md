...
Три активные вкладки: **CALENDAR** (1), **MESSENGER** (2), **NOTES** (3).
Вкладки Home, System и Cloud **не разрабатываются** — убраны из навигации.
Файлы `home.jsx`, `system.jsx`, `cloud.jsx` остались на диске, но не
подключены в `index.html`.

Вкладка NOTES открывает `WorkspaceScreen` с переключателем режимов:
- **[g] NOTES** — полноценный редактор заметок (`NotesScreen`)
- **[j] JOURNAL** — дневник (`DiaryScreen`): заметки с заголовком `YYYY-MM-DD*`

### Tauri IPC — важное правило
Tauri v2 **молча игнорирует** параметры с именем из нескольких слов (`note_id`,
`event_id`). Все параметры команд должны быть однословными: `id`, `peer`,
`field`, `since` и т.д.

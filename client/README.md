...
> ⚠️ **АРХИВ.** Этот Tauri-клиент заморожен и сохранён в ветке `tauri`. Активная
> разработка клиента продолжается как форк Telegram Desktop (см. корневой
> `README.md`, раздел «Пивот клиента»). Каталог остаётся как рабочая точка
> отката и источник стиля/моделей экранов для переноса.

#!/usr/bin/env bash
# Линковка истории в форке X (эмулятор): стек + alice-десктоп (PARVANE_AUTOLINK_GRANT=1,
# с историей от bob) → tgx_session_flow.sh (X как второе устройство alice) → X публикует
# оффер, десктоп грантует, X импортирует и показывает старое сообщение. Логи в /tmp/pv-tgx-session.

...
### Формула инкрементальной сборки (выверено 8 сен 2026)

Компиляция и линковка требуют разного. Шесть потоков компиляции проходят, а
линковка отладочного бинаря на 5 ГБ рядом с ними в 16 ГБ не помещается: две
попытки подряд сняло по нехватке памяти, причём линкер успевает УДАЛИТЬ старый
бинарь до падения — пользователь остаётся без клиента. Поэтому:

```bash
cd /home/ub/Projects/active/Parvane/desktop      # ТОЛЬКО этот путь, см. ниже
...
Линкер — `lld` (`-fuse-ld=lld` в кэше CMake, бинарь в `~/.local/bin`), пик
свопа ~13 ГБ, около получаса. Своп на 30 ГБ обязателен.

**Путь к репозиторию — строго `/home/ub/Projects/active/Parvane`.** Это симлинк
на `/mnt/hdd/ub/Projects/active/Parvane`, и кэш CMake сконфигурирован под
`/home/...`. Запуск `cmake --build` из `/mnt/hdd/...` заставляет CMake
переразрешить пути, объектники переезжают в другой подкаталог и все ~3600
готовых обесцениваются — полная пересборка на пустом месте (8 сен 2026 так и
случилось). Правка `parvane-core/CMakeLists.txt` тоже вызывает переконфигурацию,
поэтому после неё первая сборка длиннее.

`add_custom_target(parvane_e2e_build ALL)` всегда «грязный»: `ninja -n` будет
показывать 2 действия (cargo + link) даже на собранном дереве — это норма.

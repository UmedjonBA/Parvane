...
| `client` (`desktop/`) | Десктопный клиент — форк Telegram Desktop | ✅ мессенджер + звонки работают (текст с форматированием, медиа, аудио/видео/групповые звонки, группы/каналы); рескin и Календарь/Дневник — впереди |
| `smarthome` | Умный дом, RBAC по устройствам | ⛔ заморожен |

**Тесты:** unit-тесты бэкенда (`cargo test --workspace`, включая argon2-пароли) +
слой `parvane-core` клиента (transport, messenger, crypto/SAS, call_session,
call_manager, group, group_call) — все зелёные (`scripts/run_all_tests.sh`).
...
> `desktop/verify_*.sh`). Дальше — рескин (Фаза 5), Календарь/Дневник (Фаза 6),
> и пробелы паритета из `desktop/PARITY-telegram.md` (превью ссылок, стикеры,
> E2E-текст, папки/архив, админка групп). Ботов/Stories/Premium — не делаем.

...
| `client` | Десктопный клиент | 🔄 переезд на форк Telegram Desktop (см. ниже) |
...
> Дальнейший клиент строится как **форк Telegram Desktop** (`tdesktop`, C++/Qt):
> зрелый UX переключается с MTProto на наш бэкенд (NATS + Rust-шарды через
> NATS C-клиент `cnats`), затем перекрашивается под TUI/Gruvbox; КАЛЕНДАРЬ и
> ДНЕВНИК добавляются после готового мессенджера. Форк наследует лицензию
> tdesktop — **GPLv3** (с OpenSSL-исключением). План: `desktop/` (заводится по
> мере реализации).
